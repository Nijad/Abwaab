...
namespace Abwaab.Application.Features.Plans.CancelPlan
{
    public class CancelUserPlanCommandHandler : IRequestHandler<CancelUserPlanCommand, CancelUserPlanResponse>
    {
        ...
        public async Task<CancelUserPlanResponse> Handle(CancelUserPlanCommand request, CancellationToken cancellationToken)
        {
            ...
            UserPlanStatus activeStatus = await _planService.FindUserPlanStatusByStatusNameAsync(Common.Enums.UserPlanStatesEnum.Active);
            ...
            UserPlanStatus canceledStatus = await _planService.FindUserPlanStatusByStatusNameAsync(Common.Enums.UserPlanStatesEnum.Canceled);
            ...
            if (userPlanPendingPayment != null)
                userPlan.Payments!.Where(x=>x.Id==userPlanPendingPayment.Id).First().PaymentState = pendingPaymentState;

...
const VerifyChangeEmail = ({ onClose, onSuccess, newEmail }) => {
  ...
  const verify = async () => {
    ...
    try {
      ...
      const resp = await profileApi.confirmEmailChange(
        newEmail,
        code,
        signalRef.current.signal
      );
      enqueueSnackbar(resp.data.message, { variant: "success" });
      if (onSuccess) onSuccess(newEmail, resp.data);
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <DialogContentText>
          أرسلنا رمزًا إلى {newEmail}، سيبقى بريدك الحالي فعالًا حتى نجاح
          التحقق.
        </DialogContentText>
